		...
		<table>
			...
			<tbody>
				<c:forEach var="lang" varStatus="loop" items="${languages}">
					<tr>
						<td>
							<a href="/languages/${lang.id}"><c:out value="${lang.name}"/></a>
						</td>
						<td><c:out value="${lang.creator}"/></td>
						<td><c:out value="${lang.version}"/></td>
						<td>
							<a href="/languages/edit/${lang.id}">Edit</a>
							<form:form action="/languages/delete/${lang.id}" method="POST"><input class="float" type="submit" value="Delete" /></form:form>	
						</td>
					</tr>	
				</c:forEach>							
			</tbody>
